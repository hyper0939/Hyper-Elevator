...
local function HasJobAccess(source, elevator)
	if not elevator.job then
		return true
	end

	local xPlayer = ESX.GetPlayerFromId(source)

	return xPlayer and xPlayer.job.name == elevator.job
end
...
lib.callback.register("hyper_elevator:Server:RequestTeleport", function(source, elevatorKey, floorId)
	...
	if not HasJobAccess(source, elevator) then
		return { success = false, reason = Config.Languagues["no_access"] }
	end
	...
	if lastUse and (now - lastUse) < Config.TeleportCooldown then
		return { success = false, reason = Config.Languagues["cooldown"] }
	end
...
AddEventHandler("hyper_elevator:Server:CanUse", function(source, elevatorKey)
	...
	return cb(HasJobAccess(source, elevator))
end)

ESX.RegisterServerCallback("hyper_elevator:Server:RequestTeleport", function(source, cb, elevatorKey, floorId)
	local elevator = Config.Elevators[elevatorKey]
	if not elevator then
		return cb({ success = false, reason = Config.Languagues["elevator_not_found"] })
	end
	...
	if not floor then
		return cb({ success = false, reason = Config.Languagues["floor_not_found"] })
	end

	if not HasJobAccess(source, elevator) then
		return cb({ success = false, reason = Config.Languagues["no_access"] })
	end
	...
	if lastUse and (now - lastUse) < Config.TeleportCooldown then
		return cb({ success = false, reason = Config.Languagues["cooldown"] })
	end

	-- Distance check
	local ped = GetPlayerPed(source)
	if not ped or ped == 0 then
		return cb({ success = false, reason = Config.Languagues["player_not_found"] })
	end
	...
	if dist > Config.MaxTeleportDistance then
		return cb({ success = false, reason = Config.Languagues["distance"] })
	end
...
--#region Logging
RegisterNetEvent("hyper_elevator:Server:Log", function(elevatorKey, floorId) end)
--#endregion
...
local function CheckForUpdate()
	...
	PerformHttpRequest(
		("https://raw.githubusercontent.com/%s/%s/%s/hyper_elevator/fxmanifest.lua"):format(
			GITHUB_USER,
			GITHUB_REPO,
			GITHUB_BRANCH
		),
		function(status, body, headers)
			...
			if currentVersion == latestVersion then
				...
				print(
					("^3[%s] ^7Update available! Installed ^1v%s ^7=> New: ^2v%s"):format(
						ResourceName,
						currentVersion,
						latestVersion
					)
